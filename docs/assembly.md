...
```assembly
;========================================
; 求连续 uint8 数据的最大值和最小值
; 输入: R0 = 数据起始地址, R2 = 数据个数
; 输出: R3 = 最大值, R4 = 最小值
;========================================
GET_MAX_MIN:
        ; 1. 初始化：用第一个数据作为初始最大值和最小值
        MOV A, @R0          ; 取第一个数据
        MOV R3, A           ; 最大值 = 第一个数据
        MOV R4, A           ; 最小值 = 第一个数据
        INC R0              ; 指针指向下一个数据
        DEC R2              ; 计数器减1（已处理第一个）
        JZ DONE             ; 如果只有1个数据，直接返回
...
LOOP:
        ...
        ; 3. 比较最大值
        CJNE A, R3, CHECK_MAX ; 与当前最大值比较
CHECK_MAX:
        JC CHECK_MIN        ; 如果 CY=1 (A < R3)，跳过更新最大值
        MOV R3, A           ; 否则更新最大值

        ; 4. 比较最小值
CHECK_MIN:
        CJNE A, R4, CHECK_MIN2 ; 与当前最小值比较
CHECK_MIN2:
        JNC NEXT            ; 如果 CY=0 (A >= R4)，跳过更新最小值
        MOV R4, A           ; 否则更新最小值

        ; 5. 继续下一个数据
NEXT:
        INC R0              ; 指针+1
        DJNZ R2, LOOP       ; 计数器-1，不为0继续循环
