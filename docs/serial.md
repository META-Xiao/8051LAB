...
UART 是**全双工**（Full-Duplex）的通信协议，即可以同时发送和接收数据。

#### UART 的工作模式（按帧格式分）

| 模式 | 数据位 | 校验位 | 停止位 | 用途 |
|:---|:---|:---|:---|:---|
| **8N1** | 8 位 | 无 | 1 位 | 最常用，默认配置 |
| **8E1** | 8 位 | 偶校验 | 1 位 | 需要简单检错 |
| **8O1** | 8 位 | 奇校验 | 1 位 | 需要简单检错 |
| **7N1** | 7 位 | 无 | 1 位 | 兼容旧设备 |
| **9N1** | 9 位 | 无 | 1 位 | 多机通信（第9位做地址/数据标志） |

---

#### UART 的工作模式（按多机通信分）

8051 的 UART 有 **4 种工作模式**（由 SCON 寄存器 SM0/SM1 设置）：

| 模式 | SM0 SM1 | 方式 | 波特率 | 特点 |
|:---|:---|:---|:---|:---|
| **方式 0** | 0 0 | 同步移位寄存器 | fosc/12 | 扩展 IO，不常用 |
| **方式 1** | 0 1 | 10 位 UART | 可变（T1 溢出率决定） | **最常用**，8 数据 + 1 起始 + 1 停止 |
| **方式 2** | 1 0 | 11 位 UART | fosc/32 或 fosc/64 | 固定波特率，第9位可编程 |
| **方式 3** | 1 1 | 11 位 UART | 可变（T1 溢出率决定） | 多机通信，第9位 = TB8/RB8 |
